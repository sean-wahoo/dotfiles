...
local snacks_config = {
  ...
  terminal = {
    ...
    on_buf = function(self)
      keymap(modes, '<leader>tf', function() self:hide() end, "toggle focus/hide")
...
keymap(modes, '<leader>tn', function() Snacks.terminal.open() end, "new terminal")

-- for plugin, config in pairs(snacks_opts) do
--   snacks[plugin] = vim.tbl_extend('force', snacks[plugin], config)
-- end
-- snacks.setup(snacks_opts)
-- Snacks.terminal.keys = snacks_opts.terminal.keys
